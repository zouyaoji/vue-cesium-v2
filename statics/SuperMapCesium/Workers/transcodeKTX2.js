 ...
 */
define(['./when-8d13db60', './Check-70bec281', './RuntimeError-ba10bc3e', './WebGLConstants-4c11ee5f', './createTaskProcessorWorker', './PixelFormat-8e0e5be1'], function (when, Check, RuntimeError, WebGLConstants, createTaskProcessorWorker, PixelFormat) {
  ...
  function parseUncompressed(header, result) {
    ...
    for (var i = 0; i < header.levels.length; ++i) {
      ...
      for (var j = 0; j < header.faceCount; ++j) {
        ...
        var faceView;
        if (!when.defined(datatype) || PixelFormat.PixelDatatype.sizeInBytes(datatype) === 1) {
